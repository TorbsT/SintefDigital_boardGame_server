...
impl GameController {
    pub fn new(logger: Arc<RwLock<dyn Logger + Send + Sync>>) -> GameController {
        GameController { games: Vec::new(), unique_ids: Vec::new(), logger: logger }
    }
    ...
    pub fn handle_player_input(&mut self, player_input: PlayerInput) -> Result<GameState, &str> {
        ...
        let mut related_game = match games_iter.find(|game| game.id == player_input.player.connected_game_id) {
            Some(game) => game,
    ...
    fn create_new_game_and_assign_host(&mut self, new_lobby: NewGameInfo) -> Result<GameState, String> {
    ...
    fn handle_movement(input: PlayerInput, game: &mut GameState) -> Result<(), String> {
        ...
        match game.update_player(player) {
            Ok(_) => return Ok(()),
            Err(e) => return Err(format!("Failed to move player because: {}", e).to_string()),
        };
    }
...
fn make_game_controller() -> GameController {
    ...
    GameController::new(logger.clone())
}
...
pub fn get_all_wanted_unique_player_ids(amount_of_new_ids_to_create: usize) -> bool {
    ...
    for _ in 0..amount_of_new_ids_to_create {
        ...
        let id = controller.generate_player_id().unwrap();
        if ids.contains(&id) {
            return false;
         }
...
pub fn test_creating_new_games(amount_of_new_players_to_create: i32, amount_of_new_games: i32) -> bool {
    let mut controller = make_game_controller();
    
    let random_players: Vec<Player> = make_random_player_list_with_size(amount_of_new_players_to_create, &mut controller);

    let new_lobbies: Vec<NewGameInfo> = make_random_new_lobbies(amount_of_new_games, random_players);

    let mut games_created: Vec<GameState> = Vec::new();

    for new_lobby in &new_lobbies {
        match controller.create_new_game(new_lobby.clone()) {
            Ok(game) => games_created.push(game),
            Err(_) => (),
        }
    }

    if games_created.len() != min(amount_of_new_players_to_create as usize, amount_of_new_games as usize) {
        return false
    }

    let mut actual_games_to_create_from_full_list: Vec<NewGameInfo> = Vec::new();
    for i in 0..new_lobbies.len() {
        if actual_games_to_create_from_full_list.iter().any(|lobby| lobby.host.unique_id == new_lobbies.get(i).unwrap().host.unique_id) {
            continue;
        }
    ...
    for lobby in actual_games_to_create_from_full_list {
        if !games_created.iter().any(|game| {
            game.players.iter().any(|player| {
                player.unique_id == lobby.host.unique_id
...
fn make_random_new_lobbies(amount_of_new_games: i32, random_players: Vec<Player>) -> Vec<NewGameInfo> {
    ...
    for _ in 0..amount_of_new_games {
        ...
        new_lobbies.push(NewGameInfo { host: player.clone(), name: rand::random::<i32>().to_string() });
    }

    new_lobbies
}

fn make_random_player_list_with_size(amount_of_new_players_to_create: i32, controller: &mut GameController) -> Vec<Player> {
...
fn make_random_player_info(controller: &mut GameController) -> Player {
    let player: Player = Player { connected_game_id: 0, in_game_id: InGameID::PlayerOne, unique_id: get_unique_player_id(controller).unwrap(), name: rand::random::<i32>().to_string(), position: Node {id: 0, name: "".to_string(), neighbours_id: Vec::new()} };
    player
}
...
mod tests {
    ...
    fn test_player_movement() {
        ...
        let mut start_node = Node {id: 1, name: "Start".to_string(), neighbours_id: Vec::new()};
        let mut end_node = Node {id: 2, name: "End".to_string(), neighbours_id: Vec::new()};
        ...
        let lobby = NewGameInfo {host: player.clone(), name: "Test".to_string()};
        ...
        assert!(game.players.iter().any(|p| p.unique_id == player.unique_id && p.position.id == player.position.id));

        player = game.players.iter().find(|&p| p.unique_id == player.unique_id).unwrap().clone();
