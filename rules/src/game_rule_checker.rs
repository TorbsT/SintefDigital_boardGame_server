...
fn has_enough_moves(game: &GameState, player_input: &PlayerInput) -> Result<ValidationResponse, String> {
    ...
    let Some(node_id) = player_input.related_node_id else {
        return Err("The input had no node ID".to_string());
    };
    let cost = match current_node.get_movement_cost_to_neighbour_with_id(node_id) {
        Ok(cost) => cost,
        Err(e) => return Err(e),
    };
                ...
                Some(node_id) => {
                    ...
                    match map.get_node_by_id(node_id) {
                        Ok(node) => {
                            ...
                            if !node.has_neighbour_with_id(node_id) {
                                return invalid_move!(format!("The node {} is not a neighbour of the player's position!", node_id));
                            }
                        },
                        Err(e) => return Err(e),
                    };
